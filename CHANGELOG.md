...
- Reworked `.github/.codecov.yml` (which had both statuses off): the project status is informational for now, while the patch status enforces 75% coverage on new/changed code; the Codecov upload step now points at `coverage/lcov.info` explicitly and fails CI on upload errors ([#57](https://github.com/brain-bbqs/bbqs-uploader/pull/57))
- Split the Test workflow into a fail-fast `quality` job (typecheck, lint, unit tests with coverage, Codecov upload) and a separate `integration` job for Playwright, so browser runs never block the coverage upload ([#57](https://github.com/brain-bbqs/bbqs-uploader/pull/57))
...
- Added ESLint and typescript-eslint as pinned devDependencies with an `npm run lint` script, matching the versions the pre-commit hook already pins ([#57](https://github.com/brain-bbqs/bbqs-uploader/pull/57))
- Skipped the `eslint` hook on pre-commit.ci (local pre-commit runs still include it): type-aware rules need the app's `node_modules` for correct type information, which pre-commit.ci does not install, so its run saw different types and pushed a wrong auto-fix; the `npm run lint` CI step is the enforced check ([#57](https://github.com/brain-bbqs/bbqs-uploader/pull/57))
