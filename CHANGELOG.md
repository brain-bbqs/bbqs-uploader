...
- Split the Test workflow into a fail-fast `quality` job (typecheck, lint, unit tests with coverage, Codecov upload, with the upload now pointing at `coverage/lcov.info` explicitly) and a separate `integration` job for Playwright, so browser runs never block the coverage upload ([#57](https://github.com/brain-bbqs/bbqs-uploader/pull/57))
