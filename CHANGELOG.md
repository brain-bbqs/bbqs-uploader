...
- Moved the quality gates into their own fail-fast Quality workflow (typecheck, lint, unit tests with coverage, Codecov upload, with the upload now pointing at `coverage/lcov.info` explicitly), leaving the Test workflow with just the Playwright `integration` job, so browser runs never block the coverage upload ([#57](https://github.com/brain-bbqs/bbqs-uploader/pull/57))
