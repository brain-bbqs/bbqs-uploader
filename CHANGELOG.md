...
- Added an "Expand depth" slider next to the `sourcedata/raw/` heading; only the first two levels of nesting expand by default (folders with more than 30 entries still start collapsed regardless), and dragging the slider re-expands/collapses the whole tree to any depth on the fly ([#17](https://github.com/brain-bbqs/ember-uploader/pull/17))
- Added an overall progress tracker above the file list: a byte-weighted progress bar plus live counts of done/skipped/error/cancelled files for the current upload batch ([#17](https://github.com/brain-bbqs/ember-uploader/pull/17))
